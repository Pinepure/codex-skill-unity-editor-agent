...
Use the AI Unity Editor Agent as the primary execution surface for Unity work. Treat the live manifest as the source of truth, prefer existing Unity tools over any other path, and iterate capabilities intelligently by adding only the smallest missing project-internal tool when the manifest proves a real gap.

## Core Loop

1. Start every Unity task with `GET /health`, `GET /manifest`, and `compile.status`.
2. Plan from the current manifest, not memory.
3. Use existing Unity tools first.
4. Add a new tool only when the manifest lacks a sufficient existing tool or tool combination.
5. After `tool.upsert_script`, wait for compile idle, inspect compile errors if needed, reload the manifest, then call the new tool.
6. Validate the result with the relevant playbook and the validation matrix before closing the task.
...
- Never assume a tool exists before reading the latest manifest.
...
- Re-read the manifest after any tool installation or tool change.
- For write operations, verify the resulting Unity state with a manifest-advertised inspection tool whenever possible.
- When a tool fails, inspect `compile.errors` or `service.log_recent` before guessing.
